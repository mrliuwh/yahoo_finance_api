...
class StockForecastModel:
    ...
    def backtest_predictions(self, min_train_size: int = 252, step: int = 1) -> pd.DataFrame:
        """Backtest using the currently trained model without re-fitting.

        The current model predicts 1-month forward change for each historical row
        (from `min_train_size` onward), then compares predictions with actual
        1-month forward change.
        """
        if self.data is None or "target_1m_price_change" not in self.data.columns:
            self.build_features()
        if self.artifacts is None:
            raise RuntimeError("Call train() before backtest_predictions().")

        df = self.data.copy()
        X_all = df[self.artifacts.feature_columns]
        y_all = df["target_1m_price_change"]

        idx = list(range(min_train_size, len(df), step))
        if not idx:
            return pd.DataFrame(columns=["predicted_1m_change", "actual_1m_change", "prediction_error"])

        pred_arr = self.artifacts.model.predict(X_all.iloc[idx])
        backtest_df = pd.DataFrame({
            "predicted_1m_change": pred_arr,
            "actual_1m_change": y_all.iloc[idx].values,
        }, index=df.index[idx])
